        ...
        if (response) {
            if (response.code === '20200') {
                ...
                if (mainScript._purpose === 'searchSms') {
                    ...
                    document.getElementById('leaveBizMDate').innerText = data.leaveBizMDate;
                    ...
                    document.getElementById('laterBizMDate').innerText = data.laterBizMDate;
                    document.getElementById('notResponseBizMDate').innerText = data.notResponseBizMDate;
                    document.getElementById('reportBizMDate').innerText = data.reportBizMDate;
                }
                const modalEl = document.querySelector(selector);
                const modal = bootstrap.Modal.getOrCreateInstance(modalEl);
                modal.show();
        ...
        Object.keys(data).reverse()
            .forEach(function (key) {
                ...
                let values = {
                    ...
                    'leaveBizM': data[key]['LeaveBizM'] ? 'Y' : '-',
                    'diseaseBizM': data[key]['DiseaseBizM'] ? 'Y' : '-',
                    'consultBizM': data[key]['ConsultBizM'] ? 'Y' : '-',
                    'laterBizM': data[key]['LaterBizM'] ? 'Y' : '-',
                    'notResponseBizM': data[key]['NotResponseBizM'] ? 'Y' : '-',
                    'reportBizM': data[key]['ReportBizM'] ? 'Y' : '-',
                    'options': key,
                };
                for (let k in values) {
