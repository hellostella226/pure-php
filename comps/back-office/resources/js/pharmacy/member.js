    ...
    setEventListener: function () {
        ...
        document.getElementsByName('data-modify').forEach(function (btn) {
            btn.addEventListener('click', function () {
                ...
                document.getElementById('inputGender').value = tr.childNodes[8].textContent;
                document.getElementById('inputBirth').value = tr.childNodes[6].textContent;
                document.getElementById('inputPhone').value = tr.childNodes[9].textContent;
                let state = tr.childNodes[10].textContent.split(' ')[0].trim();
                document.getElementById('state').childNodes.forEach(e => {
                    if (e.innerText === state) {
                        e.selected = true;
                    }
                });
                getAddress('city', tr.childNodes[10].textContent.replace(state, '').trim());
    ...
    rendering: function (data, num) {
        ...
        Object.keys(data).forEach(function (key) {
                ...
                let values = {
                    ...
                    'LatestStatus': data[key]['ProductName'] + ':' + data[key]['LatestStatus'],
                    'Options': '',
                };
                for (let k in values) {
